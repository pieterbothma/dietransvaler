...
Afrikaanse satiriese nuus — *fopnuus wat jy kan vertrou*. Beskikbaar by [dietransvaler.co.za](https://dietransvaler.co.za).
...
2. Vul die frontmatter in — alle velde behalwe die drie `prent*`-velde is verplig:
   ...
   **Belangrik:**
   - `prentAlt` beskryf wat die prent vertoon (vir skermlesers en soekenjins).
   - `prentBronskrif` is die byskrif wat onder die prent wys — krediet en konteks.
   - Hulle is twee verskillende dinge. 'n Prent sonder `prentAlt` laat die bouproses misluk.
...
Slegte frontmatter laat die bouproses misluk met die lêernaam en die veld wat verkeerd is. Dit is 'n kenmerk — 'n stukkende artikel bereik nooit produksie nie.
...
npm run build   # verifieer dat alles nog staties bou
npm run lint    # ESLint loop oor die kode
```

Om toetse in kykmodus te loop:
...
Satiriseer instellings en tipes, nie gewone mense by naam nie. Suid-Afrika het geen parodie-verweer teen laster nie — hierdie reël hou die projek veilig.
